#!/usr/bin/env python3
"""
Blog Quality Analyzer — vendored into Warren Workflow.

Source:    https://github.com/AgriciDaniel/claude-blog (scripts/analyze_blog.py)
License:   MIT (preserved at end of file)
Author:    AgriciDaniel
Version:   1.6.9 at vendor time

Warren-side additions are isolated to the bottom of the file under
"WARREN INTEGRATION HOOKS" — do NOT modify the upstream analyzer logic
above that marker so we can re-vendor cleanly when upstream updates.
...
# ===========================================================================
# WARREN INTEGRATION HOOKS  (everything above this is upstream, do not edit)
# ===========================================================================
#
# Adds in-memory entry points so the Warren dashboard can score generated
# content without writing temporary files first.

import tempfile as _tempfile
import os as _os


def analyze_text(content: str, *, suffix: str = ".md") -> dict:
    """Score `content` (HTML or Markdown) on the 100-point rubric.

    The upstream analyzer is file-driven (`analyze_file(path)`), and rebuilding
    its full pipeline on a string would duplicate ~1500 lines. We write to a
    short-lived temp file, run `analyze_file`, then unlink — a few KB of disk
    I/O per generation, well under the LLM call we already paid for.

    Args:
        content: Raw blog content. HTML or Markdown both work; the analyzer
                 strips tags / fences before scoring.
        suffix:  File extension hint. Use ".md" for markdown, ".html" for HTML.

    Returns:
        The full analysis dict from `analyze_file`. Key fields:
          analysis['score']['total']          int 0..100
          analysis['score']['grade']          'Exceptional'|'Strong'|...
          analysis['score']['categories']     {content, seo, eeat, technical, ai}
          analysis['score']['issues']         [{category, severity, issue}]
        Returns {'error': '...'} on failure (mirrors upstream contract).
    """
    fd, path = _tempfile.mkstemp(suffix=suffix, prefix="warren-blog-quality-")
    try:
...
def quick_score(content: str, *, suffix: str = ".md") -> dict:
    """Compact summary suitable for UI cards. Returns:

    {
      "total":       int,           # 0..100
      "grade":       str,           # 'Exceptional' | 'Strong' | 'Acceptable' | ...
      "categories":  {content, seo, eeat, technical, ai},   # int per category
      "max_per_category": {...},    # max possible points per category
      "top_issues":  [{category, severity, issue}, ...],    # capped to 8
      "raw":         dict,          # full analyze_file result for deep dives
    }

    Returns {"error": ...} if analysis fails.
    """
    full = analyze_text(content, suffix=suffix)
    if "error" in full:
        return {"error": full["error"]}
    score = full.get("score") or {}

    # Upstream uses long category keys + 'rating' (not 'grade'). Normalise to
    # short keys so the dashboard UI can render predictably without coupling
    # to the upstream naming choices.
    cats_long = score.get("categories", {}) or {}
    cats = {
        "content":   int(cats_long.get("content_quality", 0)),
        "seo":       int(cats_long.get("seo_optimization", 0)),
        "eeat":      int(cats_long.get("eeat_signals", 0)),
        "technical": int(cats_long.get("technical_elements", 0)),
        "ai":        int(cats_long.get("ai_citation_readiness", 0)),
    }
    return {
        "total":       int(score.get("total", 0)),
        "grade":       score.get("rating", "Unknown"),
        "categories":  cats,
        "max_per_category": {
            "content": 30, "seo": 25, "eeat": 15, "technical": 15, "ai": 15,
        },
        "top_issues":  (score.get("issues") or [])[:8],
